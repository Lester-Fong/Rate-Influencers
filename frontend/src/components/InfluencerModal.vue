<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" :aria-labelledby="titleId">
    <div class="admin-panel max-h-[90vh] w-full max-w-3xl overflow-y-auto shadow-2xl">
      <form @submit.prevent="handleSubmit">
        <div class="flex items-center justify-between border-b px-6 py-4">
          <h2 :id="titleId" class="text-xl font-semibold">
            {{ isEditing ? "Edit influencer" : "Create influencer" }}
          </h2>
          <button type="button" class="admin-muted text-2xl hover:text-rose-500" :disabled="isLoading" aria-label="Close" @click="emit('close')">&times;</button>
        </div>

        <div class="grid gap-5 p-6 md:grid-cols-2">
          <div>
            <label for="influencer-name" class="mb-1 block text-sm font-medium">Name</label>
            <input id="influencer-name" v-model.trim="form.name" class="admin-field" type="text" @input="setSuggestedSlug" />
            <p v-if="fieldError('name')" class="mt-1 text-sm text-red-600">{{ fieldError("name") }}</p>
          </div>

          <div>
            <label for="influencer-slug" class="mb-1 block text-sm font-medium">Slug</label>
            <input id="influencer-slug" v-model.trim="form.slug" class="admin-field" type="text" placeholder="example-creator" />
            <p v-if="fieldError('slug')" class="mt-1 text-sm text-red-600">{{ fieldError("slug") }}</p>
          </div>

          <div class="md:col-span-2">
            <label for="influencer-bio" class="mb-1 block text-sm font-medium">Bio</label>
            <textarea id="influencer-bio" v-model.trim="form.bio" class="admin-field min-h-24" maxlength="2000"></textarea>
            <p v-if="fieldError('bio')" class="mt-1 text-sm text-red-600">{{ fieldError("bio") }}</p>
          </div>

          <div class="md:col-span-2">
            <label for="profile-picture" class="mb-1 block text-sm font-medium">Profile picture URL</label>
            <input id="profile-picture" v-model.trim="form.profile_picture" class="admin-field" type="url" placeholder="https://example.com/avatar.jpg" />
          ...
          <div v-for="field in socialFields" :key="field.key">
            ...
            <input :id="field.key" v-model.trim="form[field.key]" class="admin-field" type="url" placeholder="https://example.com/profile" />
        ...
        <div class="flex justify-end gap-3 border-t px-6 py-4">
          <button type="button" class="admin-theme-button px-4 py-2" :disabled="isLoading" @click="emit('close')">Cancel</button>
